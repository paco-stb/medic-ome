    ...
    onAuthStateChanged(auth, async (user) => {
        ...
        if (user) {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    if (data.lastUpdate && data.lastUpdate.seconds) {
                        ...
                        if (diffDays >= 3) setTimeout(() => showAlert(`👋 Bon retour ${state.pseudo} !`, "success"), 1000);
                    }
                    updateHeader();
                    updateStreakDisplay();
                    renderHome();
            ...
            if (!state.isGuest && savedGuest) {
                ...
                state.progression = { ...state.progression, ...JSON.parse(savedGuest) };
                updateHeader();
                renderHome();
            } else {
...
function renderHome() {
    ...
    const btnIA = document.createElement('button'); btnIA.className = 'btn'; btnIA.style.flex = '1'; btnIA.style.background = 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)'; btnIA.innerHTML = '<i class="ph-duotone ph-brain"></i> Diagnostic IA (Indisponible pour le moment)';
    ...
    btnIA.onclick = () => { const limit = checkGuestLimit(); if (!limit.allowed) { showAlert(`Mode invité limité !`, "error"); return; } if(audioCtx.state === 'suspended') audioCtx.resume(); state.useLLM = true; state.dailyTarget = null; renderDemographics(); }; 
